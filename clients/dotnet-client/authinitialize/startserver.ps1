...
if ($GFSH_PATH -ne "")
{
    ...
    Invoke-Expression "$GFSH_PATH  -e 'start locator --name=locator $LOCATOR_OPTS' "
    Invoke-Expression "$GFSH_PATH -e 'connect --locator=localhost[10334] --user=server --password=server' -e 'start server --name=server $COMMON_OPTS'"
    Invoke-Expression "$GFSH_PATH -e 'connect --locator=localhost[10334] --user=server --password=server' -e 'create region --name=region --type=PARTITION'"
}
