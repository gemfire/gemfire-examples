...
if ($GFSH_PATH -ne "")
{
   Invoke-Expression "$GFSH_PATH -e 'start locator --name=locator --dir=$PSScriptRoot/locator' -e 'deploy --jar=../utilities/example.jar' -e 'start server --name=server0 --server-port=40505' -e 'create region --name=partition_region --type=PARTITION'"
}