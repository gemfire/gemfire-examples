...
if ($GFSH_PATH -ne "")
{
   Invoke-Expression "$GFSH_PATH -e 'connect --use-ssl=true --key-store=$PSScriptRoot\keys\keystore.jks --trust-store=$PSScriptRoot\keys\truststore.jks --trust-store-password=password --key-store-password=password' -e 'shutdown --include-locators=true'"
}
